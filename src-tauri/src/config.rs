...
///   且不一定等于用户目录，可能导致 `.302-cc-switch/cc-switch.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取应用配置目录路径 (~/.302-cc-switch)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    // 302 fork 独占 ~/.302-cc-switch，与原版 cc-switch 的 ~/.cc-switch 完全隔离。
    // 上游 v3.10.3 的 HOME 环境变量旧库回退逻辑对本 fork 不适用，已移除。
    get_home_dir().join(".302-cc-switch")
}
