...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.302-cc-switch/crash.log）
        ...
        .setup(|app| {
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes under the app's com.ai302.ccswitch data directory.
            ...
            // 使用平台对应的托盘图标（macOS 使用 302 模板图标适配深浅色）
            #[cfg(target_os = "macos")]
            {
